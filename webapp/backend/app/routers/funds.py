...
from app.models import Fund, FundMetric, MonthlyReturn, ConfirmedGap
...
def _to_response(fund: Fund, session: Session, gap_count: int | None = None) -> FundResponse:
    """ORM Fund -> FundResponse，附带数据截止年月与是否有指标。

    data_cutoff_month 直接取最新 monthly_return 月份，不用 fund_metrics 缓存值
    （缓存可能在 skills 摄取新月份后过期）。
    gap_count 由调用方批量查询传入避免 N+1；None 时单基金即时查。
    """
...
def list_funds(session: Session = Depends(get_db)):
    ...
    return [_to_response(f, session, gap_count=gap_map.get(f.fund_id, 0)) for f in funds]
