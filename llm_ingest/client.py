...
Spec G: web_search grounding 支持 (Client.messages_with_search /
_parse_grounding / follow_redirect / resolve_sources) 已删除 -- 命中率仅
53%, 会 hallucinate URL, grounding 跳板需直连 Google 国内网络卡死。搜索
统一改走 llm_ingest/search.py (Tavily) 与 llm_ingest/grok.py (Grok)。
本客户端现只负责纯文本/PDF messages 调用。
"""
